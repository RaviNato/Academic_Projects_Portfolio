...
boolean guarde_na_fila (Fila* f, ElementoDeFila e){
    if ((*f).qtd_atual==(*f).capacidade){
        ...
        if ((*f).inicio != 0 && (*f).final != 0){
            for (I32 i = (*f).inicio, p = 1; p <= ((*f).qtd_atual-(*f).inicio); i++, p++){
...
boolean free_fila (Fila* f)
{
    ...
    for (I32 i=(*f).inicio; (*f).qtd_atual>0; (*f).qtd_atual--, (*f).inicio = (*f).inicio+1==(*f).capacidade?0:(*f).inicio+1){
        (*f).vetor[i]=NULL;
        free((*f).vetor[i]);
    }
